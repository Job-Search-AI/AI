...
<p align="center">
  <img src="img/job-search-search-flow-desktop.gif" alt="채용공고 검색 플로우 데스크톱 데모" width="960">
</p>
...
| 노드 | 역할 | 핵심 출력 |
| --- | --- | --- |
| 요청 접수 | 사용자 요청이 들어와 전체 워크플로우를 시작함 | 요청 접수 이벤트 |
| LLM 인스턴스 준비 | 이후 단계들이 공통으로 사용할 LLM 인스턴스 또는 추론 컨텍스트를 준비함 | 초기화된 모델 객체 / 실행 컨텍스트 |
| 질문에서 조건 추출 | 사용자 질문에서 회사명, 직무, 지역, 경력, 기술스택 같은 조건을 추출함 | 추출된 조건 엔티티 |
| 조건 해석 및 정규화 | 추출된 조건을 표준 형식으로 정리하고, 모호한 표현을 해석하며, 정보 부족 여부를 판정함 | 정규화된 검색 조건 또는 추가 정보 필요 상태 |
| 대상 공고 URL 결정 | 정규화된 조건을 바탕으로 직접 조회할 대상 공고 URL 또는 소스 경로를 결정함 | 대상 공고 URL |
| 공고 페이지 수집 | 결정된 URL의 채용 공고 페이지를 실제로 수집함 | 원본 HTML 문서 |
| 공고 핵심 정보 추출 | HTML에서 직무명, 회사명, 위치, 요구사항, 우대사항, 마감일 등 핵심 정보를 구조화함 | 구조화된 공고 정보 |
| 관련 공고 통합 검색 | 추출된 공고 정보를 기준으로 유사 공고나 관련 공고를 통합 검색함 | 관련 공고 목록 / 랭킹 결과 |
| 사용자 답변 생성 | 원 공고 정보와 관련 공고 검색 결과를 종합해 사용자에게 보여줄 답변을 만듦 | 최종 사용자 응답 텍스트 |
| 처리 종료 | 정상 종료 또는 추가 정보 부족에 따른 조기 종료를 처리함 | 처리 종료 상태 |
...
| 원인 | `latest` 태그만 갱신하고 실행 이미지 확인 절차가 없으면, 배포 상태가 불명확해질 수 있었습니다.                           |
| 해결 | Cloud Build에서 VM 재시작 후 실행 이미지와 `latest`를 함께 확인하는 롤아웃 검증 절차를 추가했습니다.                    |
| 결과 | 배포 후 실제 실행 중인 이미지가 최신 배포와 일치하는지 빠르게 확인할 수 있게 되었고, 롤아웃 확인 절차가 단순해졌습니다. |
...
`main push -> Cloud Build trigger -> Docker image build -> Artifact Registry push -> VM rollout -> health check`
...
- Cloud Build는 이미지 업로드 후 VM 재시작과 실행 이미지 확인까지 자동화합니다.
...
- 실행 이미지 확인 절차를 붙여 `배포는 됐는데 실제 반영은 안 된 상태`를 빠르게 확인할 수 있게 했습니다.
